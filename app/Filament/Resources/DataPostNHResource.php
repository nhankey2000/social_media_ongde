...
use App\Filament\Resources\DataPostNHResource\Pages;
...
class DataPostNHResource extends Resource
{
    protected static ?string $model = DataPostNH::class;
        ...
        return $form
            ->schema([
                ...
                Forms\Components\Hidden::make('existing_files')
                    ->visible(fn (?DataPostNH $record): bool => $record !== null),
            ]);
    }
    ...
    {
        return [
             'index' => Pages\ListDataPostNH::route('/'),
             'create' => Pages\CreateDataPostNH::route('/create'),
             'edit' => Pages\EditDataPostNH::route('/{record}/edit'),
        ];
    }
    ...
    {
        // Xóa files cũ
        DataImagesNH::where('post_id', $post->id)->delete();
    ...
    {
        return DataImagesNH::where('post_id', $post->id)
            ->pluck('url')
            ->map(function ($url) {
