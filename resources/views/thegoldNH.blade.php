...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .vip-card {
            background: linear-gradient(135deg, rgba(218, 165, 32, 0.92), rgba(255, 215, 0, 0.92));
            color: #ffffff;
        ...
        .vip-card:hover {
            background: linear-gradient(135deg, rgba(218, 165, 32, 0.98), rgba(255, 215, 0, 0.98)) !important;
...
<div class="container">
    ...
    <h1 class="main-title">GOLD VIP CARD</h1>
    ...
    <div class="social-grid">
        <div class="social-card vip-card" onclick="showVipPrivileges()">
            <div class="social-icon">
                <img src="images/vipgold.png" alt="Đặc Quyền VIP" loading="lazy">
            </div>
            <div class="social-title">Đặc Quyền VIP GOLD</div>
            <div class="social-description">Ưu đãi độc quyền cho thành viên GOLD</div>
        </div>
    ...
    <div id="vipModal" class="menu-modal">
        <div style="max-width: 500px; background: white; border-radius: 16px; padding: 20px; position: relative; box-shadow: 0 10px 30px rgba(0, 0, 0, 0.3);">
            ...
            <div style="margin-bottom: 20px;">
                <img id="vip-image" src="images/vipgold.png" alt="GOLD VIP" style="width: 80px; height: 80px; border-radius: 50%; border: 3px solid #ffd700; box-shadow: 0 0 20px rgba(255,215,0,0.5);">
            </div>
            <h2 id="vip-title" style="margin: 16px 0; color: #d32f2f; font-weight: 800; font-size: 1.5rem;">
                ĐẶC QUYỀN THẺ GOLD VIP
            </h2>
            ...
            <p style="margin-top: 24px; font-size: 0.9rem; color: #666; font-style: italic;">
...
<script>
    ...
    function showVipPrivileges() {
        ...
        contentBox.innerHTML = '<p style="margin:0; color:#b71c1c;"><strong>Đang tải ưu đãi...</strong></p>';
        ...
        fetch(`${window.location.origin}/api/vip-card/info`)
            ...
            .then(result => {
                ...
                const goldCard = result.data.find(card => card.type === 'GOLD');

                if (!goldCard) {
                    throw new Error('Không tìm thấy thẻ GOLD');
                }

                document.getElementById('vip-image').src = 'images/vipgold.png';
                document.getElementById('vip-title').textContent = 'ĐẶC QUYỀN THẺ GOLD VIP';

                if (contentBox && goldCard.content) {
                    const temp = document.createElement('div');
                    temp.innerHTML = goldCard.content;
                ...
                expirySpan.innerHTML = `Đến hết ngày <u>${goldCard.expiry_date}</u>`;
            })
            .catch(err => {
