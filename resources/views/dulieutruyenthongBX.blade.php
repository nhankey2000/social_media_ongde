...
<div class="container">
    <div class="logo-section">
        <div class="logo" style="width: 160px; height: 96px; margin: 0 auto 20px; border-radius: 20px; background: #ffffff; display: flex; align-items: center; justify-content: center; box-shadow: 0 16px 32px rgba(0, 0, 0, 0.1);">
            <img src="images/logo.png" alt="Logo Bánh Xèo Cô Tư" style="width: 128px; height: 64px; border-radius: 12px;" onerror="this.style.display='none'">
        </div>
    </div>

    <h1 class="header-title">🍳 Bánh Xèo Cô Tư</h1>
...
<script>
    ...
    // API endpoints - Updated for BX tables
    ...
    // Load categories - Updated for BX tables
    async function loadCategories() {
        try {
            const response = await fetch(`${API_BASE_URL}/api/danhmuc-bxs`);
    ...
    // Select content with category - Updated for BX tables
    ...
    // Load data with category filter - Updated for BX tables
    async function loadData(type, categoryId = 'all') {
        ...
        try {
            ...
            if (categoryId === 'all') {
                if (type === 'posts') {
                    url = `${API_BASE_URL}/api/data-posts-bx`;
                } else {
                    url = `${API_BASE_URL}/api/data-images-bx?type=${type === 'images' ? 'image' : 'video'}`;
                }
            } else {
                url = `${API_BASE_URL}/api/danhmuc-bxs/${categoryId}/${type}`;
            }
    ...
    function renderTable(items, type) {
        ...
        if (type === 'posts') {
            ...
            rows = shuffledItems.map(item => `
                <tr>
                    <td><strong>${item.title && item.title.length > 5 ? item.title.substring(0, 5) + '...' : (item.title || 'Không có tiêu đề')}</strong></td>
            ...
            columns = ['Preview', 'Ngày Tạo', 'Hành Động'];
            rows = shuffledItems.map(item => `
                <tr>
                    <td>
                        ${item.url ?
